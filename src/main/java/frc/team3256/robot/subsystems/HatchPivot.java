...
public class HatchPivot extends SubsystemBase {
	...
	private boolean atClosedLoopTarget() {
		if (!m_usingClosedLoop || stateChanged)
			return false;
	...
	public static class PivotFloorIntakePreset extends RobotState {
		...
		public RobotState update() {
			...
			if (HatchPivot.getInstance().atClosedLoopTarget()) {
				...
				HatchPivot.getInstance().setTargetPosition(ControlMode.Position, HatchPivot.getInstance().m_closedLoopTarget, Constants.kHatchPivotDownSlot);
				return new IdleState();
			} else
				return new IdleState();
		}
	...
	public static class PivotDeployPreset extends RobotState {
		...
		public RobotState update() {
			...
			if (HatchPivot.getInstance().atClosedLoopTarget()) {
				...
				HatchPivot.getInstance().setTargetPosition(ControlMode.Position, HatchPivot.getInstance().m_closedLoopTarget, Constants.kHatchPivotUpSlot);
				return new IdleState();
			} else
				return new IdleState();
		}
