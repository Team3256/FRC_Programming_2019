...
public class CargoIntakeStateBased extends SubsystemBase {
    ...
    private static CargoIntakeStateBased instance;

    public static CargoIntakeStateBased getInstance() {
        return instance == null ? instance = new CargoIntakeStateBased() : instance;
    }

    private PivotStateBased mPivot = PivotStateBased.getInstance();
    private Elevator mElevator = Elevator.getInstance();

    private CargoIntakeStateBased() {
...
//                } else
    ...
    private SystemState handleIntake() {
        ...
        mPivot.setWantedState(PivotStateBased.WantedState.WANTS_TO_INTAKE_POS);

        if (mStateChanged) {
            startTime = Timer.getFPGATimestamp();
            System.out.println(mPivot.isBrakeEngaged());
    ...
    private SystemState handleExhaust() {
        ...
        if (mStateChanged) {
            mPivot.setWantedState(PivotStateBased.WantedState.WANTS_TO_EXHAUST_POS);
        }
        setIntakePower(-kIntakeSpeed);
        return defaultStateTransfer();
    }
    ...
    private SystemState handleStop() {
        if (mStateChanged) {
            mPivot.setWantedState(PivotStateBased.WantedState.WANTS_TO_DEPLOY_POS);
            setIntakePower(0);
        }
    ...
    private SystemState defaultStateTransfer() {
        System.out.println(mWantedState);
                switch (mWantedState) {
